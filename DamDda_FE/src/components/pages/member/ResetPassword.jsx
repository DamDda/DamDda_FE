...
const ResetPassword = () => {
  const [formData, setFormData] = useState({ id: "", name: "", email: "", password: "", password_confirm: "" });
  const [errors, setErrors] = useState({ id: "", name: "", email: "", password: "", password_confirm: "" });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setErrors({ id: "", name: "", email: "", password: "", password_confirm: "" });
    ...
    try {
      const response = await axios.put(`${SERVER_URL}member/findpw`, {
        headers: {
  ...
  const handleResetPassword = () => {
    ...
    if (formData.password.length < 8 || formData.password.length > 16) {
      setErrors((prev) => ({ ...prev, password: "비밀번호는 8-16자리로 입력해주세요." }));
      return;
    }
    if (formData.password !== formData.password_confirm) {
      setErrors((prev) => ({ ...prev, password_confirm: "비밀번호가 일치하지 않습니다." }));
      return;
    }
