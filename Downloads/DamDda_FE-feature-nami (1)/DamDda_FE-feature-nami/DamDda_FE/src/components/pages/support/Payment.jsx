...
function Payment() {
  ...
  const handleSubmit = async () => {
    ...
    const orderData = {
      ...
      supportingPackages: orderInfo.selectedPackages.map((pkg) => ({
        packageDTO: {
          id: pkg.id,
          name: pkg.packageName,
          price: pkg.price,
          quantityLimited: pkg.quantityLimited,
          RewardList: pkg.RewardList.reduce((acc, r) => {
            const optionKey = `${pkg.id}-${r.id}`;
            const selectedOption = pkg.selectedOption[optionKey];
            if (selectedOption) {
              acc.push({
                id: r.id,
                name: r.name,
            ...
            <div className="order-summary">
              ...
              {orderInfo.selectedPackages &&
                orderInfo.selectedPackages.map((pkg, index) => {
                  ...
                  console.log("구분------------------------------");
                  console.log(orderInfo); // 개별 패키지 구조 확인

                  return (
                    <div key={index}>
                      선물구성: {pkg.packageName} /
                      {pkg.RewardList &&
                        pkg.RewardList.map((reward, rewardIndex) => (
                          <span key={rewardIndex}>
                            선물: {reward.name} 옵션:
                            {pkg.selectedOption[`${pkg.id}-${reward.id}`]} /
                          </span>
                        ))}
                      가격: {pkg.count * pkg.price} 원 / 수량: {pkg.count} 개
                      <br />
                    </div>
                  );
                })}
          ...
          <div className="small-container">
            <div className="payment-summary">
              ...
              <div className="payment-summary-amount">
                <p>상품 금액: {orderInfo.totalAmount} 원</p>{" "}
                {/* 상품 금액은 수량 곱하기 */}
                <p>배송비: 3000 원</p>
                <p>최종 금액: {orderInfo.totalAmount + 3000} 원</p>{" "}
                {/* 최종 금액 */}
              </div>
            </div>
            ...
            <button onClick={handleSubmit} className="button-2">
              간편 결제 하기
            </button>
          </div>
        </div>
