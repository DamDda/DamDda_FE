...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // 데이터베이스에서 가져오기
      // const response = await fetch('/api/gift-options'); // DB에서 받아오는 API 경로
      ...
      const details = [
        { id: 1, text: "상세 설명 내용", image: "https://via.placeholder.com/800" },
      ];
      ...
      <DetailSection>
        ...
        {detailedDescription.map(detail => (
          <div key={detail.id} style={{ marginBottom: "20px" }}>
            <Typography variant="body1" style={{ marginTop: "10px" }}>{detail.text}</Typography>
            <ImageContainer src={detail.image} alt={`Detail Image ${detail.id}`} />
              ...
              <div>
                {pkg.options.map((option) => (
                  <div key={option}>
                    <Checkbox
                      checked={
                        selectedOptions[pkg.id]?.includes(option) || false
                      }
