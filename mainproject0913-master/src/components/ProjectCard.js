  ...
  return (
    <>
      ...
      >
        ...
        {/* 중앙 텍스트 정보 */}
        <CardContent sx={{ flex: 1, marginRight: '5px', padding: '8px', margin: '0px' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'flex-end', minWidth: '160px' }}>
          {/* 상단에 후원번호와 결제날짜 */}
          <Box sx={{ textAlign: 'right', marginBottom: '80px' }}>
            <Typography level="body2" sx={{ mb: 1, fontSize: '0.875rem', color: 'text.secondary' }}>
              후원번호: {project.supportNumber}
            </Typography>
            <Typography level="body2" sx={{ fontSize: '0.875rem', color: 'text.secondary' }}>
              결제 날짜: {new Date(project.paymentDate).toLocaleString()}
            </Typography>
          </Box>

          {/* 하단에 진행 상태와 결제 정보 버튼 */}
          <Box sx={{ display: 'flex', gap: '5px', justifyContent: 'flex-end', marginTop: 'auto' }}>
      ...
      {/* 결제/배송 정보 표시 */}
      {showDetails && <PaymentDeliveryInfo project={paymentData} />} {/*가짜데이터 보여주기*/}
