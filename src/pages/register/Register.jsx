...
const Register = () => {
    ...
    const fetchWriteData = async () => {
        try {
            ...
            const response = await axios
                ...
                .catch((error) => console.error('프로젝트 데이터를 가져오는 중 오류 발생:', error));
    ...
    const saveProject = async (projectId, submit, isSkip = false) => {
        ...
        // productImages 파일 추가
        addFileToForm(projectFormData, productImages, 'productImages', 'productImagesMeta', 'updateProductImages');
                ...
                <div className="header-container">
                    ...
                    <div className="button-right">
                        <div className="button-group">
                            ...
                            <BlueButtonComponent text="저장하기" onClick={() => saveProject(projectId, '저장')} />
                        </div>
                    </div>
                </div>
                    ...
                    <Box sx={style}>
                        <Preview
                            ...
                            project={{
                                ...
                                targetFunding: parseInt(formData.target_funding.replace(/,/g, '')),
                                startDate: formData.start_date,
                                endDate: formData.end_date,
                                category: formData.category_id,
                                descriptionDetail: descriptionDetail,
                                productImages: productImages.map((image) => getUrl(image)),
                                descriptionImages: descriptionImages.map((image) => getUrl(image)),
                            }}
                        />
                    </Box>
                </Modal>
