...
const ProjectDetail = ({
  ...
  // 버튼 클릭 시 호출될 함수
  console.log("Project ID:", projectId); // Project ID 확인selectedOption
  ...
  const handleNavigateToPayment = () => {
    const orderInfo = {
      ...
      selectedPackages: selectedPackages.map((pkg) => ({
        ...
        salesQuantity: pkg.salesQuantity,
  ...
  const fetchPackage = async () => {
    try {
      ...
      const formattedPackages = response.data.map((pac) => ({
        ...
        salesQuantity: pac.salesQuantity,
  ...
  const handleSelectPackage = () => {
    if (selectedPackage) {
      ...
      selectedPackage.RewardList.forEach((reward, index) => {
        const key = `${selectedPackage.id}-${reward.id}`;
        if (selectedOptions[key]) {
          selectedOptionsList[key] = selectedOptions[key];
        }
      });
  ...
  const handleSelectOption = (pkgId, rewardIndex, option) => {
    setSelectedOptions((prevOptions) => ({
      ...prevOptions,
      [`${pkgId}-${rewardIndex}`]: option,
    }));
  };
                      ...
                      {reward.OptionList && reward.OptionList.length > 0 ? (
                        <FormControl fullWidth>
                          ...
                          <Select
                            ...
                            value={
                              selectedOptions[`${pkg.id}-${reward.id}`] || ""
                            }
                            onChange={(event) => {
                              handleSelectOption(
                                pkg.id,
              ...
              <SelectedCard key={`${pkg.id}-${pkg.selectedOption}`}>
                <div>
                  ...
                  {pkg.selectedOption && (
                    <Typography variant="body2">
                      옵션:{" "}
                      {Object.entries(pkg.selectedOption).map(
                        ([key, value], index) => (
                          <span key={key}>
                            {index > 0 && ", "}
                            {`${key.split("-")[1]}: ${value}`}
